...
export async function participantsUpdate({id, participants, action}) {
  if ((this.opts || global.opts || {})['self']) return
  ...
  switch (action) {
    ...
    case 'remove':
      if (chat.welcome) {
        let groupMetadata = await this.groupMetadata(id).catch(() => ({}))
        for (let user of participants) {
          ...
          let text = (
            action === 'add'
              ? (chat.sWelcome || 'مرحباً @user!').replace('@subject', groupMetadata?.subject || id).replace('@desc', groupMetadata?.desc?.toString() || '𝐺𝐴𝑅𝐴 𝐵𝛩𝑇')
              : (chat.sBye || 'وداعاً @user!')
          ).replace('@user', '@' + user.split('@')[0])
          this.sendMessage(id, { text, contextInfo: { mentionedJid: [user], externalAdReply: { showAdAttribution: true, renderLargerThumbnail: true, thumbnail: apii?.data || null, title: global.namebot || '𝐺𝐴𝑅𝐴 𝐵𝛩𝑇', mediaType: 1 } } }, { quoted: null })
        }
      }
      break
    case 'promote':
    case 'demote':
      let text2 = (action === 'promote' ? (chat.sPromote || '@user أصبح أدمن') : (chat.sDemote || '@user لم يعد أدمن')).replace('@user', '@' + participants[0].split('@')[0])
      if (chat.detect) this.sendMessage(id, { text: text2, mentions: this.parseMention(text2) })
      break
  }
}
...
export async function callUpdate(callUpdate) {
  ...
  for (let nk of callUpdate) {
    if (nk.isGroup == false && nk.status == 'offer') {
      await this.reply(nk.from, `مرحباً @${nk.from.split('@')[0]}، المكالمات غير مسموح بها.`, false, { mentions: [nk.from] }).catch(() => {})
...
export async function deleteUpdate(message) {
  try {
    ...
    await this.sendMessage(msg.chat, { text: `*رسالة محذوفة من @${participant?.split('@')[0]}*`, mentions: [participant] }, { quoted: msg })
    this.copyNForward(msg.chat, msg).catch(() => {})
