...
import { gaaraJadiBot } from './plugins/jadibot.js';
...
import socketNodes from './lib/socketNodes.js'
...
const welcomeText = `🌙 Welcome to the core of GARA Bot 🌙\nPreparing your session... GARA cannot wait to serve you, dear user ✨`;
...
if (connection == 'open') {
console.log(chalk.bold.green('\n❀ GARA-Bot Connected successfully ❀'))
    socketNodes(global.conn)
}
let reason = new Boom(lastDisconnect?.error)?.output?.statusCode
if (connection === 'close') {
if (reason === DisconnectReason.badsessions) {
console.log(chalk.bold.cyanBright(`\n⚠︎ NO CONNECTION, DELETE THE FOLDER ${global.sessions} AND SCAN THE QR CODE ⚠︎`))
...
console.log(chalk.bold.yellowBright(`\n╭┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄ ✗\n┆ ⚠︎ CONNECTION REPLACED, ANOTHER NEW SESSION HAS BEEN OPENED, PLEASE CLOSE THE CURRENT SESSION FIRST.\n╰┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄ ✗`))
...
console.log(chalk.bold.redBright(`\n⚠︎！ UNKNOWN DISCONNECT REASON: ${reason || 'Not found'} >> ${connection || 'Not found'}`))
}}
}
...
// Native boot for subbots by - ReyEndymion >> https://github.com/ReyEndymion

global.rutaJadiBot = join(__dirname, './GARAjadibot')

if (global.GARAJadiBots) {
...
GARAJadiBot({pathGARAJadiBot: botPath, m: null, conn, args: '', usedPrefix: '/', command: 'serbot'})
}
}
}
}
