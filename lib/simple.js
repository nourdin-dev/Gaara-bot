...
// proto وباقي الدوال كلها named exports مباشرة
const _makeWaSocket = baileys.makeWASocket
...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    let sock = Object.defineProperties(conn, {
        ...
        // ─── decodeLid ────────────────────────────────────────────────────────
        decodeLid: {
            async value(lid, groupId = null) {
                if (!lid?.endsWith('@lid')) return lid;
                const lidNum = lid.split('@')[0];

                // الطريقة 1: group metadata
                if (groupId?.endsWith('@g.us')) {
                    try {
                        const meta = await this.groupMetadata(groupId);
                        for (const p of (meta?.participants || [])) {
                            if (
                                ...
                                if (p.jid && !p.jid.endsWith('@lid') && p.jid.includes('@')) {
                                    return p.jid;
                                }
                            }
                        }
                ...
                // الطريقة 2: signalRepository
                try {
                    const pn = await this.signalRepository?.lidMapping?.getPNForLID?.(lid);
                    if (pn) return pn.includes('@') ? pn : `${pn}@s.whatsapp.net`;
                } catch (_) {}

                // الطريقة 3: contacts / chats
                try {
                    const store = this.contacts || this.chats || {};
                    for (const [contactId, c] of Object.entries(store)) {
                        ...
                        if (
                            ...
                            if (contactId && !contactId.endsWith('@lid') && contactId.includes('@')) {
                                return contactId;
                            }
                        }
                    }
    ...
    value(jid = '', withoutContact = false) {
        ...
        if (jid.endsWith('@g.us')) {
            ...
            return conn.groupMetadata(jid)
                .then(m => m?.name || m?.subject || jid.split('@')[0])
                .catch(() => jid.split('@')[0])
        }
        ...
        // البوت نفسه
        if (areJidsSameUser(jid, conn.user?.id || conn.user?.jid || '')) {
            return conn.user?.name || conn.user?.verifiedName || conn.user?.notify || jid.split('@')[0]
        }

        // مستخدم عادي - نجرب كل المصادر
        const v = conn.chats[jid] || {}
        const fromStore = store?.contacts?.[jid]
        const fromDB = global.db?.data?.users?.[jid]?.name

        return withoutContact ? ''
            : fromDB || v.name || v.notify || v.vname || v.verifiedName
            || fromStore?.name || fromStore?.notify || fromStore?.verifiedName
...
export async function smsg(conn, m, hasParent) {
    ...
    try {
        ...
        if (rawSender?.endsWith('@lid')) {
            const resolved = await conn.decodeLid(rawSender, m.isGroup ? m.chat : null);
            if (resolved) {
        ...
        console.error('[smsg] decodeLid error:', e.message);
    }
...
export function serialize() {
    ...
    this.message?.templateButtonReplyMessage?.selectedDisplayText ||
    this.message?.buttonsResponseMessage?.selectedButtonId ||
    this.message?.interactiveResponseMessage?.nativeFlowResponseMessage?.paramsJson && 
        (() => { try { return JSON.parse(this.message.interactiveResponseMessage.nativeFlowResponseMessage.paramsJson)?.id } catch { return '' } })() ||
    ''
const interactiveText = this.message?.interactiveResponseMessage?.nativeFlowResponseMessage?.paramsJson
    ? (() => { try { return JSON.parse(this.message.interactiveResponseMessage.nativeFlowResponseMessage.paramsJson)?.id || '' } catch { return '' } })()
    : ''
return typeof this._text === 'string' ? this._text : '' || interactiveText || (typeof text === 'string' ? text : (
